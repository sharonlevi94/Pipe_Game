                ...
                switch (randVal) {
                    case STRAIGHT_PIPE_E:
                        m_map[x][y] = (std::make_unique<Rotatable>(sf::Vector2f
                                                                           (boxSize.x * y + 64, boxSize.y * x + 64) +
                                                                   this->m_location, boxSize, STRAIGHT_PIPE_E,
                                                                   STRAIGHT_PIPE_F, false));
                        break;
                    case T_PIPE_E:
                        m_map[x][y] = (std::make_unique<Rotatable>(sf::Vector2f
                                                                           (boxSize.x * y + 64, boxSize.y * x + 64) +
                                                                   this->m_location, boxSize, T_PIPE_E, T_PIPE_F,
                                                                   false));
                        break;
                    case PLUS_PIPE_E:
                        m_map[x][y] = (std::make_unique<Rotatable>(sf::Vector2f
                                                                           (boxSize.x * y + 64, boxSize.y * x + 64) +
                                                                   this->m_location, boxSize, PLUS_PIPE_E, PLUS_PIPE_F,
                                                                   false));
                        break;
                    case CORNER_PIPE_E:
                        m_map[x][y] = (std::make_unique<Rotatable>(sf::Vector2f
                                                                           (boxSize.x * y + 64, boxSize.y * x + 64) +
                                                                   this->m_location, boxSize, CORNER_PIPE_E,
                                                                   CORNER_PIPE_F, false));
                        break;
                    default:
                        m_map[x][y] = (std::make_unique<Rotatable>(sf::Vector2f
...
void Board::setSinks(const sf::Vector2f &boxSize) {
    for (int sink = 0; sink < 1; sink++) {
        ...
        this->m_map[randRow][randCol] = std::make_unique<Sink>(sf::Vector2f
                                                                       (boxSize.x * randCol + 64,
                                                                        boxSize.y * randRow + 64) + this->m_location,
                                                               boxSize, SINK_E, SINK_F, false);
...
sf::Vector2f Board::setFaucet(const sf::Vector2f &boxSize) {
    ...
    this->m_map[randRow][randCol] = std::make_unique<Rotatable>(sf::Vector2f
                                                                        (boxSize.x * randCol + 64,
                                                                         boxSize.y * randRow + 64) + this->m_location,
                                                                boxSize, FAUCET_F, FAUCET_F, true);
            ...
            if (start.x != end.x) { //diff rows
                if (end.y == start.y) { //same cols
                    if (end.x > start.x) { //end is below to start
                        ...
                        if (start == end) {
                            ...
                            this->m_map[start.x][start.y] =
                                    std::make_unique<Rotatable>(sf::Vector2f
                                                                        (boxSize.x * start.y +
                                                                         64,
                                                                         boxSize.y * start.x +
                                                                         64) + this->m_location,
                                                                boxSize, STRAIGHT_PIPE_E,
                                                                STRAIGHT_PIPE_F, false);
                        }
                ...
                if (end.y < start.y) { // end above to start
                    ...
                    if (start == end) {
                        ...
                        this->m_map[start.x][start.y] =
                                std::make_unique<Rotatable>(sf::Vector2f
                                                                    (boxSize.x * start.y + 64,
                                                                     boxSize.y * start.x + 64) +
                                                            this->m_location, boxSize,
                                                            STRAIGHT_PIPE_E, STRAIGHT_PIPE_F,
                                                            false);
                    }
                } else {
                    start.y++;
