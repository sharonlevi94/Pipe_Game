...
Controller::Controller():
    ...
    m_board(Board(sf::Vector2f(600, 100), sf::Vector2f(800, 800))){
    ...
    while (m_window.isOpen())
    {
        ...
        if (auto event = sf::Event{}; m_window.waitEvent(event))
        {
            ...
            case sf::Event::MouseButtonPressed:
                if ((Rotatable*)(this->m_board.getContent(sf::Vector2f((float)event.mouseButton.x,
                    (float)event.mouseButton.y))))
                    this->m_board.getContent(sf::Vector2f((float)event.mouseButton.x,
                        (float)event.mouseButton.y))->rotate();
                break;
            default:
